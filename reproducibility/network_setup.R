...
# osm cycle
# 
# london_cycle_edges <- st_read() |> as.data.table()
# 
# london_cycle_nodes <- st_read() |> as.data.table()
# 
# osm_cycle <- make_cppr_net(london_cycle_edges[,.(from,to,length)]
#                          ,nodes = london_cycle_nodes[,.(osmid,x,y)])
# 
# osm_cycle |> rlist::list.save()
#   
# # osm walk
# 
# london_walk_edges <- st_read() |> as.data.table()
# 
# london_walk_nodes <- st_read() |> as.data.table()
# 
# osm_walk <- make_cppr_net(london_walk_edges[,.(from,to,length)]
#                            ,nodes = london_walk_nodes[,.(osmid,x,y)])
# 
# osm_walk |> rlist::list.save()

## DODGR NETWORK

# london_all_edges_lcc <- get_lcc(london_all_edges)
