...
london_nodes_dt <- st_read("RC_outputs/london_all/london_all.gpkg"
                           ,layer = "nodes") |> 
  as.data.table()
...
london_msoa <- list.load("data/london_msoa.rds") |> as.data.table()
...
ind_of_interest <- find_nearest_node_on_graph(graph=network_tidygraph |> activate(nodes)|> select(osmid,x,y) |> as.data.frame()
                                              ,points_data = london_msoa[,"centr_geom"]
                                              ,return_id = FALSE
                                              ,wkt=1,crs=4326)

#### sf_networks if left as it causes memory faults. 
# 
# network_sf <- tryCatch(rlist::list.load("native_networks/network_sf.rds")
#                        ,error = function(e) e
#                        ,warning = function(w) w
#                        ,finally = cat('If could not read the data, will generate it.'))
# 
# if(inherits(network_sf,c('warning','error'))){
#   network_sf <- sfnetworks::as_sfnetwork(network_tidygraph)
#   network_sf |> rlist::list.save("benchmarks/native_networks/network_sf.rds")
# }
# 
...
from_xy <- london_msoa[,"centr_geom"] |> st_as_sf(wkt=1,crs=4326) |> st_coordinates()

from_ind <- dodgr::match_points_to_graph(dodgr_cycle, from_xy, connected = TRUE)
